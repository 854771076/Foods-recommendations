        ...
        <el-main class="main">
            ...
            <el-card :class="{ 'folded': !card2, '': card2 }" style="margin-top: 15px;">
                ...
                <el-form ref="UserresumeruleFormRef" :model="UserresumeruleForm" label-width="90px " :size="formSize"
                    ...
                    <el-form-item label="类型" class="address">
                        <el-select v-model="UserresumeruleForm.type1" class="m-2" placeholder="类型1">
                            <el-option v-for="item in this.$baseData.type_code" :key="item.name" :label="item.name"
                                :value="item.code" />
                        </el-select>
                        <el-select v-model="UserresumeruleForm.type2" class="m-2" placeholder="类型2">
                            <el-option v-for="item in this.$baseData.type_code" :key="item.name" :label="item.name"
                                :value="item.code" />
                        </el-select>
                        <el-select v-model="UserresumeruleForm.type3" class="m-2" placeholder="类型3">
                            <el-option v-for="item in this.$baseData.type_code" :key="item.name" :label="item.name"
                                :value="item.code" />
                        </el-select>
                    </el-form-item>
...
export default {
    ...
    watch: {
        ...
        UserresumeruleForm: {
            handler(item1, item2) {
                this['UserresumeruleForm']['type1Translation'] = this.getname(this.$baseData['type_code'], item1['type1'])
                this['UserresumeruleForm']['type2Translation'] = this.getname(this.$baseData['type_code'], item1['type2'])
                this['UserresumeruleForm']['type3Translation'] = this.getname(this.$baseData['type_code'], item1['type3'])
        ...
        resumeinfo: {
            ...
            handler(item1, item2) {
                ...
                this['UserresumeruleForm']['type1'] = parseInt(!item1['type1'] ? 0 : item1['type1'])
                this['UserresumeruleForm']['type1Translation'] = item1['type1Translation']
                this['UserresumeruleForm']['type2'] =parseInt( !item1['type2'] ? 0 : item1['type2'])
                this['UserresumeruleForm']['type2Translation'] = item1['type2Translation']
                this['UserresumeruleForm']['type3'] = parseInt(!item1['type3'] ? 0 : item1['type3'])
                this['UserresumeruleForm']['type3Translation'] = item1['type3Translation']
