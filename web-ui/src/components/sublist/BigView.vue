<template>
  ...
  export default {
    ...
    created() {
      ...
      script.src = 'src/assets/js/flexible.js';
      document.body.appendChild(script);
      let link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = 'src/assets/css/charts.css';
    ...
    methods: {
      getinfo() {
        ...
        this.$http.get(this.$api.foods+'bigdata_info/', {
      ...
      async Right3() {
        await this.$http.get(this.$api.foods+'foods_routype_info/', {
      ...
      async Left3() {
        await this.$http.get(this.$api.foods+'foods_type_info/', {
      ...
      async Right1() {
        await this.$http.get(this.$api.foods+'star_info/', {
        ...
        var option = {
          ...
          series: [{
            ...
            data: this.right1_info.map((x)=>{
              return (Number(x.value)/Number(this.right1.total)*100).toFixed(2)
            }),
            // 显示柱子内的百分比文字
            label: {
              show: true,
      ...
      async Left1() {
        await this.$http.get(this.$api.foods+'like_info/', {
